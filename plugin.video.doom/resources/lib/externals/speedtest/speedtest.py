...
class Speedtest(object):
    ...
    def get_best_server(self, servers=None):
        ...
        # [GAIACODE]
        # HTTPSConnection does not work with Kodi's proxy. Use urllib instead.
        import urllib,datetime
        for server in servers:
            cum = []
            url = os.path.dirname(server['url']) + '/latency.txt'
            printer('%s %s' % ('GET', url), debug=True)
            for _ in range(0, 3):
                try:
                    start = datetime.datetime.now()
                    response = urllib.urlopen(url)
                    if response: r = response.read()
                    diff = datetime.datetime.now() - start
                    total = (diff.days * 86400000) + (diff.seconds * 1000) + (diff.microseconds / 1000)
                    response.close()
                except:
                    e = sys.exc_info()[1]
                    printer('%r' % e, debug=True)
